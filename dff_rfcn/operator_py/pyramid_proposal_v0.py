...
class PyramidProposalOperator(mx.operator.CustomOp):
    def __init__(self, feat_strides, scales, ratios, output_score,
                 rpn_pre_nms_top_n, rpn_post_nms_top_n, threshold, rpn_min_size):
        super(PyramidProposalOperator, self).__init__()
        self._feat_strides = np.fromstring(feat_strides[1:-1], dtype=int, sep=',')
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        score_list = []
        stride16_flag = 0
        for s in self._feat_strides:
            stride = int(s)
            sub_anchors = generate_anchors(base_size=stride, scales=self._scales, ratios=self._ratios)

            if stride == 16 and stride16_flag != 0:
                scores = cls_prob_dict['stride' + str(s) + '_p4'].asnumpy()[:, self._num_anchors:, :, :]
                bbox_deltas = bbox_pred_dict['stride' + str(s) + '_p4'].asnumpy()
            else:
                if stride == 16:
                    stride16_flag += 1
...
class PyramidProposalProp(mx.operator.CustomOpProp):
    def __init__(self, feat_strides='(32, 16, 16, 8, 4)', scales='(8)', ratios='(0.5, 1, 2)', output_score='False',
        ...
        self._feat_strides = feat_strides
        self._scales = scales
        self._ratios = ratios
    ...
    def list_arguments(self):
        arg_list = []
        flag_1 = 0
        for s in np.fromstring(self._feat_strides[1:-1], dtype=int, sep=','):
            if s == 16 and flag_1 == 1 :
                arg_list.append('rpn_cls_prob_stride' + str(s) + '_p4')
            else:
                if s == 16:
                    flag_1 = 1
                arg_list.append('rpn_cls_prob_stride' + str(s))
        for s in np.fromstring(self._feat_strides[1:-1], dtype=int, sep=','):
            if s == 16 and flag_1 == 2 :
                arg_list.append('rpn_bbox_pred_stride' + str(s) + '_p4')
            else:
                if s == 16:
                    flag_1 = 2
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return PyramidProposalOperator(self._feat_strides, self._scales, self._ratios, self._output_score,
