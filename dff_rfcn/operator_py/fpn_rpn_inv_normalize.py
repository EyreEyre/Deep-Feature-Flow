...
class RPNInvNormalizeOperator(mx.operator.CustomOp):
    def __init__(self, num_anchors, bbox_mean, bbox_std):
        ...
        self._bbox_mean = mx.ndarray.Reshape(mx.nd.array(bbox_mean), shape=(1,2,1))
        self._bbox_std = mx.ndarray.Reshape(mx.nd.array(bbox_std), shape=(1,2,1))

    def forward(self, is_train, req, in_data, out_data, aux):
        bbox_pred = in_data[0]
        tile_shape = (bbox_pred.shape[0], 2, bbox_pred.shape[2])
...
@mx.operator.register('fpn_rpn_inv_normalize')
class RPNInvNormalizeProp(mx.operator.CustomOpProp):
    def __init__(self, num_anchors, bbox_mean='(0.0, 0.0, 0.0)', bbox_std='0.1, 0.1, 0.2'):
