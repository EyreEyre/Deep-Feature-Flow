...
class FPNROIPoolingOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if is_train:
            ...
            # change mx.nd.ROIPooling --> mx.nd.contrib.PSROIPooling
            else:
                ...
                with autograd.train_section():
                    for i in range(self.num_strides):
                        ...
                        self.roi_pool[i] = mx.nd.contrib.PSROIPooling(data=in_data[i], rois=mx.nd.array(rois_p[i], in_data[i].context), group_size=7,
                                                                      pooled_size=7, output_dim=self.output_dim, spatial_scale=1.0 / self.feat_strides[i])
            ...
            if self.with_deformable:
                ...
                for i in range(self.num_strides):
                    ...
                    self.roi_pool[i] = mx.nd.contrib.PSROIPooling(data=in_data[i], rois=mx.nd.array(rois_p[i], in_data[i].context), group_size=7, pooled_size=7,
                                                                  output_dim=self.output_dim, spatial_scale=1.0 / self.feat_strides[i])
...
@mx.operator.register('fpn_bbox_roi_pooling')
class FPNROIPoolingProp(mx.operator.CustomOpProp):
    def __init__(self, feat_strides='(4,8,16,16)', pooled_height='7', pooled_width='7', with_deformable='False', output_dim='8'):
        ...
        self.with_deformable = with_deformable == False
