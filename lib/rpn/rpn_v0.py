...
def assign_pyramid_anchor(feat_shapes, gt_boxes, im_info, cfg, feat_strides=(4, 8, 16, 16, 32),
...
def assign_pyramid_anchor(feat_shapes, gt_boxes, im_info, cfg, feat_strides=(4, 8, 16, 16, 32),
    ...
    for feat_id in range(len(feat_strides)):
        ...
        if len(scales.shape) == 1:
            base_anchors = generate_anchors(
                base_size=feat_strides[feat_id], ratios=ratios, scales=scales)
        else:
            assert len(scales.shape) == len(ratios.shape) == 2
            base_anchors = generate_anchors(
                base_size=feat_strides[feat_id],
    ...
    if gt_boxes.size > 0:
        ...
        overlaps = bbox_overlaps(
            fpn_anchors.astype(np.float), gt_boxes.astype(np.float))
        argmax_overlaps = overlaps.argmax(axis=1)
        max_overlaps = overlaps[np.arange(len(fpn_anchors)), argmax_overlaps]
        gt_argmax_overlaps = overlaps.argmax(axis=0)
    ...
    if balance_scale_bg:
        ...
        for feat_id in range(0, len(feat_strides)):
            ...
            if len(bg_ind_scale) > num_bg_scale:
                disable_inds = npr.choice(
                    bg_ind_scale,
                    size=(len(bg_ind_scale) - num_bg_scale),
                    replace=False)
                fpn_labels[disable_inds] = -1
    else:
        if len(bg_inds) > num_bg:
    ...
    for feat_id in range(0, len(feat_strides)):
        ...
        labels = labels.reshape((1, feat_height, feat_width, A)).transpose(
            0, 3, 1, 2)
        labels = labels.reshape((1, A * feat_height * feat_width))

        bbox_targets = bbox_targets.reshape((1, feat_height, feat_width,
                                             A * 4)).transpose(0, 3, 1, 2)
        bbox_targets = bbox_targets.reshape((1, A * 4, -1))
        bbox_weights = bbox_weights.reshape((1, feat_height, feat_width,
                                             A * 4)).transpose((0, 3, 1, 2))
        bbox_weights = bbox_weights.reshape((1, A * 4, -1))
